...
Assumed you have an angular component named `FeelPayButtonComponent`: 
...
In this example `ngx-script-loader` is used to handle loading the external script. Once the script is loaded, the `initailizeFeelPay` method is called to initialize the FeelPayWidget as before.